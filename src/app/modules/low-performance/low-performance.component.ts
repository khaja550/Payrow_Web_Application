...
export class LowPerformanceComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.getLowCustomers(this.selected);
    }
    ...
    reportDownload() {
        ...
        this.report_title = 'Low Performance';
    ...
    async getLowCustomers(month: any) {
        let did = 7123897
        this.months.map((mdata: any) => {
            if (month === mdata.month) {
                let num = 1
                this.customers.getLowCustDetails(mdata.id).subscribe(data => {
                    console.log(data)
                    data.lowCustomers.map((data: any) => {
