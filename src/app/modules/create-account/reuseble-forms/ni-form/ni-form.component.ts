...
@Component({
  selector: 'app-ni-form',
  templateUrl: './ni-form.component.html',
  styleUrls: ['./ni-form.component.scss']
})
export class NiFormComponent implements OnInit {
	@Input() captureData:any;
	sampleOne :any ;
	objSheet:any;
	liChecked:any=false;
	passChecked: boolean = false;
	emChecked: boolean=false;
	constructor() { }
	...
	getData(){
		...
		if(this.captureData.documentsData.length >0){
			this.captureData.documentsData.map((d:any)=>{
				if(d.documentType === "licenseDocument"){
					this.liChecked =true;
				}
				if(d.documentType === "emiratesDocument")
					this.emChecked = true;
				if(d.documentType === "passportDocument")
					this.passChecked =true;
				
			})
		}
		this.objSheet={
			merchntName : `${this.captureData.firstName}`+' '+`${this.captureData.lastName}`,
			merchntTradeName:this.captureData.firstName,
			address: this.captureData.addressDetails,
			poAdress:`${this.captureData.city}`+' '+`${this.captureData.country}`,
			mobileNo:this.captureData.mobileNumber,
			emiratesId:this.captureData.emiratesId,
			globalUserRole:this.captureData.globalUserRole,
			email:this.captureData.emailId,
			location:`${this.captureData.city}`+' '+`${this.captureData.country}`,
			tranByYear:`${'$'}${this.captureData.businessDetails['annualTurnOver']}`,
			IbanNo:this.captureData.bankDetails[0].ibanNumber,
			bankNamendBranch:`${this.captureData.bankDetails[0].bankName}`+','+`${this.captureData.bankDetails[0].branchName}`
		}
	}
	...
	async openPDF() {
		...
		let DATA5: any = document.getElementById('htmlFive');
			...
			html2canvas(DATA2).then((cn2)=>{
				...
				html2canvas(DATA3).then((cn3)=>{
					...
					html2canvas(DATA4).then((cn4)=>{
						...
						html2canvas(DATA5).then((cn5)=>{
							const FILEURI5 = cn5.toDataURL('image/png');
							...
							PDF.addPage();
							PDF.addImage(FILEURI5, 'PNG', 40, 40, (setWidth*.62) - 190, (setHeight*.62) - 90);
